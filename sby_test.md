# 历史文档：SBY Test 修复要求与执行计划

> 本文记录 `tswn_core 0.5.0` 之前使用 `tswn_case_miner` 对齐旧执行器与 JS
> 产物的排查流程。0.5.0 已删除该 binary、旧执行器和执行器选择项；下方命令仅供
> 历史复盘，不可用于当前分支。当前正式行为门禁统一运行：
>
> ```powershell
...
以下内容保留当时的任务语气与命令，仅用于复盘 0.5.0 迁移过程。
