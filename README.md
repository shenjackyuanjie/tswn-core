...
`fight`（包括 `--out-raw`）、`diff`、`raw`（包括 `!test!` 评分/胜率）和独立 `bench` 默认使用 Runtime；需要和旧实现对账时，可在仍提供 runtime 参数的命令上显式传入 `--runtime legacy`。C、Python、WASM 的无 runtime 参数高层评分/胜率接口也默认使用 Runtime，低层 legacy 对象仅作为兼容 API 保留。
