...
> 归档说明：已删除源码的链接改为保留路径与行号的历史定位文本，避免误导为当前可用源码入口；分析内容不变。
...
- `post_kill` 通过 run_post_kill（历史定位：`crates/tswn_core/src/player/skill/store.rs#L653`） 分阶段执行
- toolchain 依赖 nightly（历史定位：`rust-toolchain.toml#L2`） 与 mutable-noalias=no（历史定位：`.cargo/config.toml#L7`）

但更深入地沿着当前调用链和 trait 签名向下看，会得到更准确的现状判断：

1. 已经明确修掉的，是 provenance 问题和 `post_kill` 的同实体别名。
2. `update_state` 主路径也已经局部去别名，因为 update_states（历史定位：`crates/tswn_core/src/player/impl_attr.rs#L211`） 现在走的是 update_state_inline（历史定位：`crates/tswn_core/src/player/skill.rs#L215`），而不是运行期再次从 `Storage` 把 owner 借出来。
...
当前 `Storage` 的核心结构在 storage.rs（历史定位：`crates/tswn_core/src/engine/storage.rs#L64`） 到 storage.rs（历史定位：`crates/tswn_core/src/engine/storage.rs#L93`），关键点是：

- 玩家表是 `UnsafeCell<FastHashMap<PlrId, UnsafeCell<Player>>>`
- 运行期还有一个 `in_post_damage_player` 标记，专门用于使魔分摊伤害时校正死亡顺序

摘自 storage.rs（历史定位：`crates/tswn_core/src/engine/storage.rs#L64-L93`）：
...
摘自 just_get_player_mut（历史定位：`crates/tswn_core/src/engine/storage.rs#L336`）：
...
- `OnDamageFunc`：见 player/mod.rs（历史定位：`crates/tswn_core/src/player/mod.rs#L109`）
- `SkillArgs`：见 player/skill.rs（历史定位：`crates/tswn_core/src/player/skill.rs#L163`）
- `StateTrait::on_pre_defend` / `on_post_damage`：见 state.rs（历史定位：`crates/tswn_core/src/player/state.rs#L80`） 与 state.rs（历史定位：`crates/tswn_core/src/player/state.rs#L120`）

摘自 player/mod.rs（历史定位：`crates/tswn_core/src/player/mod.rs#L109`） 与 player/skill.rs（历史定位：`crates/tswn_core/src/player/skill.rs#L163`）：
...
- pre_defend（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1710`）
- post_defend（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1845`）
- attacked（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1900`）
- defned（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1936`）
- damage（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1962`）
- on_damaged（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L2040`）
- on_die_impl（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L2096`）

`damage()` 的关键片段如下，摘自 impl_runtime.rs（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1962-L2023`）：
...
相比最早的“裸强转 `&self -> *mut Storage`”思路，当前 Storage（历史定位：`crates/tswn_core/src/engine/storage.rs#L64`） 的 `UnsafeCell` 设计至少把 provenance 问题从“显式无效的可写来源”变成了“显式声明的内部可变性”。
...
摘自 run_post_kill（历史定位：`crates/tswn_core/src/player/skill/store.rs#L653`）：
...
主路径对它的调用在 impl_runtime.rs（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L2136`） 与 impl_runtime.rs（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L2246`）。

这不是纸面修复，而是有行为锚点的：

- merge_kill_applies_owner_growth（历史定位：`crates/tswn_core/src/player/test/minions.rs#L46`） 覆盖了 kill 后 owner 增长逻辑
- [perf/ub-fix-no-debug.md](../perf/reports/ub-fix-no-debug.md) 记录了 `330cd46` 后消除了 27 个 release-only diff failures
...
当前 `Player::update_states()` 在 impl_attr.rs（历史定位：`crates/tswn_core/src/player/impl_attr.rs#L211`） 中已经改为：
...
而 trait 本身也明确给了一个 inline 入口，见 skill.rs（历史定位：`crates/tswn_core/src/player/skill.rs#L215`）：
...
需要注意的是，`SkillStorage::update_state()` 这个旧接口仍然还在，见 skill/store.rs（历史定位：`crates/tswn_core/src/player/skill/store.rs#L332`），测试里也仍有直接调用，例如 merge_kill_applies_owner_growth（历史定位：`crates/tswn_core/src/player/test/minions.rs#L68`） 与 同函数后续的 update_state 调用（历史定位：`crates/tswn_core/src/player/test/minions.rs#L97`）。但它已经不再是战斗主循环的主通道。

### 3.4 使魔分摊伤害已经使用“局部阶段标记”修顺序

`Storage` 里的 in_post_damage_player（历史定位：`crates/tswn_core/src/engine/storage.rs#L93`） 与辅助方法 set_in_post_damage（历史定位：`crates/tswn_core/src/engine/storage.rs#L231`）、clear_in_post_damage（历史定位：`crates/tswn_core/src/engine/storage.rs#L234`）、is_in_post_damage（历史定位：`crates/tswn_core/src/engine/storage.rs#L238`） 是另一条很重要的先例：

- 它不是全局 EventQueue
- 但它已经承认“有些路径必须分阶段标记，否则死亡顺序会错”

对应的使用点在 summon.rs（历史定位：`crates/tswn_core/src/player/skill/act/summon.rs#L244-L272`）：
...
- owner_death_marks_linked_minion_for_cleanup（历史定位：`crates/tswn_core/src/player/test.rs#L1268`）
- owner_death_removes_linked_minions_in_roster_order（历史定位：`crates/tswn_core/src/player/test.rs#L1302`）
...
- self.skills.pre_action(...)（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L218`）
- skill.act(...)（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L459`）
- run_post_action_chain(...)（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L537`） 与它的调用点 impl_runtime.rs#L510（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L510`）
- self.skills.pre_defend(...)（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1710`）
- clear_positive_runtime(...)（历史定位：`crates/tswn_core/src/player/skill/store.rs#L593`）

代表性热点如下：

| 阶段                         | 外层持有的引用        | 代表实现                                                                                                                                                | 重新借回的是谁 | 说明                                                                                                                                           |
| ---------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `act`                        | owner 的 `&mut self`  | ChargeSkill（历史定位：`crates/tswn_core/src/player/skill/act/charge.rs#L51`）                                                                                   | owner          | `skill.act(...)` 在 impl_runtime.rs（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L459`） 期间执行，`charge` 又 `just_get_player_mut(args.0)` |
| `act`                        | owner 的 `&mut self`  | AccumulateSkill（历史定位：`crates/tswn_core/src/player/skill/act/accumulate.rs#L62`）                                                                           | owner          | 与 `charge` 同类，owner 仍存活时再次取 owner                                                                                                   |
| `pre_action` / `post_action` | owner 的 `&mut self`  | HideSkill（历史定位：`crates/tswn_core/src/player/skill/skl/hide.rs#L92`） / hide.rs#L121-L135（历史定位：`crates/tswn_core/src/player/skill/skl/hide.rs#L121-L135`）     | owner          | `hide` 在 `pre_action` 与 `update_state` 路径都会再次借 owner                                                                                  |
| `pre_defend`                 | target 的 `&mut self` | ReflectSkill（历史定位：`crates/tswn_core/src/player/skill/skl/reflect.rs#L23`）                                                                                 | target/owner   | `reflect` 的 owner 就是当前 defender，自借最明显                                                                                               |
| `clear_positive_runtime`     | owner 的 `&mut self`  | ChargeSkill（历史定位：`crates/tswn_core/src/player/skill/act/charge.rs#L100`） / AccumulateSkill（历史定位：`crates/tswn_core/src/player/skill/act/accumulate.rs#L103`） | owner          | 清状态时又回头借 owner                                                                                                                         |
...
最直观的例子是 ChargeSkill::act_with_level（历史定位：`crates/tswn_core/src/player/skill/act/charge.rs#L51-L59`）：
...
而它的外层调用点正是 Player::action（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L213`）。也就是说，这一类问题并不是“理论上 trait 太宽”，而是已经真正在主战斗路径里发生。
...
| ---------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------- |
| `on_absorb`                              | absorb.rs#L54（历史定位：`crates/tswn_core/src/player/skill/act/absorb.rs#L54`）     | absorb.rs#L58（历史定位：`crates/tswn_core/src/player/skill/act/absorb.rs#L58`）                                                                  | caster   | 吸血后给 caster 回血               |
| `on_berserk`                             | berserk.rs#L86（历史定位：`crates/tswn_core/src/player/skill/act/berserk.rs#L86`）   | berserk.rs#L137（历史定位：`crates/tswn_core/src/player/skill/act/berserk.rs#L137`）                                                              | target   | 给 target 叠狂暴状态               |
| `on_curse`                               | curse.rs#L94（历史定位：`crates/tswn_core/src/player/skill/act/curse.rs#L94`）       | curse.rs#L166（历史定位：`crates/tswn_core/src/player/skill/act/curse.rs#L166`）                                                                  | target   | 给 target 叠诅咒状态               |
| `on_disperse`                            | disperse.rs#L84（历史定位：`crates/tswn_core/src/player/skill/act/disperse.rs#L84`） | disperse.rs#L92（历史定位：`crates/tswn_core/src/player/skill/act/disperse.rs#L92`）                                                              | target   | 清 positive runtime/state，并扣 MP |
| `on_fire`                                | fire.rs#L71（历史定位：`crates/tswn_core/src/player/skill/act/fire.rs#L71`）         | fire.rs#L75（历史定位：`crates/tswn_core/src/player/skill/act/fire.rs#L75`）                                                                      | target   | 给 target 增加火焰状态             |
| `on_fire`（复用）                        | summon.rs#L236（历史定位：`crates/tswn_core/src/player/skill/act/summon.rs#L236`）   | fire.rs#L75（历史定位：`crates/tswn_core/src/player/skill/act/fire.rs#L75`）                                                                      | target   | 召唤技能复用了火球的 on_damage     |
| `on_ice`                                 | ice.rs#L104（历史定位：`crates/tswn_core/src/player/skill/act/ice.rs#L104`）         | ice.rs#L163（历史定位：`crates/tswn_core/src/player/skill/act/ice.rs#L163`）                                                                      | target   | 冻结状态写入，且有二次重借         |
| `on_poison`                              | poison.rs#L44（历史定位：`crates/tswn_core/src/player/skill/act/poison.rs#L44`）     | poison.rs#L113（历史定位：`crates/tswn_core/src/player/skill/act/poison.rs#L113`）                                                                | target   | 中毒状态写入，且有二次重借         |
| `lazy_attack_on_damage -> lazy_infect`   | lazy.rs#L118（历史定位：`crates/tswn_core/src/player/boss/lazy.rs#L118`）            | lazy.rs#L132（历史定位：`crates/tswn_core/src/player/boss/lazy.rs#L132`） -> lazy.rs#L186（历史定位：`crates/tswn_core/src/player/boss/lazy.rs#L186`）     | target   | 懒癌感染                           |
| `covid_spread_on_damage -> covid_infect` | covid.rs#L341（历史定位：`crates/tswn_core/src/player/boss/covid.rs#L341`）          | covid.rs#L349（历史定位：`crates/tswn_core/src/player/boss/covid.rs#L349`） -> covid.rs#L370（历史定位：`crates/tswn_core/src/player/boss/covid.rs#L370`） | target   | 新冠感染，并对全场存活玩家调移动点 |
...
- ../rust-toolchain.toml#L2（历史定位：`rust-toolchain.toml#L2`） 选择 `nightly`
- ../.cargo/config.toml#L7（历史定位：`.cargo/config.toml#L7`） 开启 `-Z mutable-noalias=no`

而配置文件本身也已经把原因写得很清楚，见 ../.cargo/config.toml（历史定位：`.cargo/config.toml#L2-L6`）：
...
- [perf/ub-fix-no-debug.md](../perf/reports/ub-fix-no-debug.md) 记录了 `330cd46` 后 UB 修复本身没有可测量性能退化
...
- 现在的 `damage()` 在 impl_runtime.rs#L1962-L2023（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L1962-L2023`）
- `on_damage` 之后的后续逻辑基本都在 on_damaged（历史定位：`crates/tswn_core/src/player/impl_runtime.rs#L2040`）
- 也就是说，天然就有一个“core damage / callback / after damage”三段式边界

更重要的是，当前代码里已经存在“先改 HP，再独立调用 `on_damaged()`”的先例：

- exchange.rs#L149（历史定位：`crates/tswn_core/src/player/skill/act/exchange.rs#L149`）
- half.rs#L123（历史定位：`crates/tswn_core/src/player/skill/act/half.rs#L123`）
...
- 中央改动集中在 impl_runtime.rs（历史定位：`crates/tswn_core/src/player/impl_runtime.rs`）
...
- ChargeSkill::act_with_level（历史定位：`crates/tswn_core/src/player/skill/act/charge.rs#L51`）
- AccumulateSkill::act_with_level（历史定位：`crates/tswn_core/src/player/skill/act/accumulate.rs#L62`）
- HideSkill::pre_action / update_state（历史定位：`crates/tswn_core/src/player/skill/skl/hide.rs#L92-L135`）
- ReflectSkill::pre_defend_with_level（历史定位：`crates/tswn_core/src/player/skill/skl/reflect.rs#L23-L80`）
...
例如 IceSkill::score_target（历史定位：`crates/tswn_core/src/player/skill/act/ice.rs#L29`） 的大部分逻辑只是在读：
...
1. update_state_inline（历史定位：`crates/tswn_core/src/player/skill.rs#L215`） 说明把技能作用直接投到 `PlayerStatus` 是可行的
2. IceSkill::score_target（历史定位：`crates/tswn_core/src/player/skill/act/ice.rs#L29`） 说明一大类只读逻辑本来就更适合基于“拆开的数据视图”表达
...
- on_damaged_triggers_on_die（历史定位：`crates/tswn_core/src/player/test/skills.rs#L99`）
- merge_and_zombie_kill_write_target_states（历史定位：`crates/tswn_core/src/player/test/minions.rs#L4`）
- merge_kill_applies_owner_growth（历史定位：`crates/tswn_core/src/player/test/minions.rs#L46`）
- zombie_kill_marks_corpse_and_queues_minion_spawn（历史定位：`crates/tswn_core/src/player/test/minions.rs#L104`）
- owner_death_marks_linked_minion_for_cleanup（历史定位：`crates/tswn_core/src/player/test/minions.rs#L575`）
- owner_death_removes_linked_minions_in_roster_order（历史定位：`crates/tswn_core/src/player/test/minions.rs#L701`）
- ice_score_halves_already_frozen_targets（历史定位：`crates/tswn_core/src/player/test/skills.rs#L1105`）
...
1. [perf/ub-fix-no-debug.md](../perf/reports/ub-fix-no-debug.md) 已经说明 `330cd46` 引入的 `run_post_kill` 没有可测量性能退化。
...
- `damage()` 的结构天然适合拆
- 有 exchange.rs#L149（历史定位：`crates/tswn_core/src/player/skill/act/exchange.rs#L149`） 和 half.rs#L123（历史定位：`crates/tswn_core/src/player/skill/act/half.rs#L123`） 这种现成先例
...
- 有 update_state_inline（历史定位：`crates/tswn_core/src/player/skill.rs#L215`） 作为现成成功范式
