...
这份记录把当前 Runtime v2 速度固定为后续回归基线，同时把普通 score 的同口径 legacy 速度固定为必须追赶的验收线。机器可读数据见 [`runtime_v2_0.4.0_baseline.json`](runtime_v2_0.4.0_baseline.json)。
...
- score：固定单线程，每个输入组 1000 场；仅 `mario` 为了兼容历史口径执行 13000 场。每组 v2/legacy 共做 5 次独立运行，并交替先跑的 runtime；
...
| fixed30 自动线程 overall | 5.872 µs/场 | 上次 v2：8.978 µs/场 | 快 34.60% | 新回归基线 |
| win-rate 单线程 13000 场 | 0.184 s | legacy：0.217784 s | 快 15.51% | v2 已快于 legacy |
...
| 输入 | 总场数 | v2 wall | v2 init | v2 fight | legacy wall 目标 | legacy init | legacy fight | v2 相对 legacy | 还需压缩 | 结果对账 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
...
score 的验收规则固定为：相同输入、相同场数、单线程且结果完全一致时，Runtime v2 的整批 wall 必须不高于同一工具测得的 legacy wall。当前 v2 数值只作为回归基线，不代表性能任务完成。历史 CLI/probe 的 mario legacy `1.113 s` 属于不同计时边界，仅保留作历史参考，不能替代本轮更严格的裸时间目标 `0.819091 s`。
...
本轮 CQP 单人第 13 组还发现了“聚气触发时没有刷新待生效疾走倍率”的真实正确性问题，已由提交 `5067a97` 修复并增加精确回归。修复后上述三种输入各重复 5 次，所有 v2/legacy 结果均一致。

## 5. win-rate 与 CQP/CQD 自动调度

win-rate 使用 `left@red` 对 `right@blue`，单线程 13000 场。Runtime v2 五次 wall 为 `0.183 / 0.184 / 0.185 / 0.183 / 0.184 s`，中位数 `0.184 s`；init/fight 中位数分别为 `0.040 / 0.142 s`，结果始终为 `7077/13000`。历史同口径 legacy 中位数为 wall `0.2177835 s`、init `0.046990 s`、fight `0.152587 s`，当前 v2 wall 快约 15.51%。

CQP/CQD 自动调度六档沿用已正式留档的同机结果，完整输入、目标号库、线程策略和逐轮口径见 [`cqp_runtime_v2_baseline.md`](cqp_runtime_v2_baseline.md)：

| 输入 | 精度 | Runtime v1 | Runtime v2 | 提升 |
| --- | ---: | ---: | ---: | ---: |
...
cargo run -p tswn_core --release --features "no_debug aux_bins" --bin track_perf_cases -- `
  ...
  --out-dir target/perf_cases_v2_0.4.0_t1 `
  --bench-runs 13000 `
  --thread 1 -q
...
cargo run -p tswn_core --release --features "no_debug aux_bins" --bin track_score_perf -- `
  ...
  --label cqp-single-v2-0.4.0 `
  --count 1000 `
  --engine both `
  --first v2 `
  --mode normal `
  --out target/score_perf_cqp_single_v2_0.4.0.json
...
正式复测 score 时应在不同进程中运行 5 次，并交替使用 `--first v2` / `--first legacy`，避免固定先后顺序把热机或频率波动偏向某一方。
