...
按"把 benchmark 各种 case 都放进训练集"的思路做了对照实验：

- 默认训练集 = `fixed_cases_30` 30 个 case + `score.txt`；
- 扩展训练集 = 默认集 + 之前的非训练样本（`aaa/bbb`、复杂 1v1、2v2），
  外加两个全新留出样本（不训练）做泛化对照。

两个 PGO 二进制（默认集 vs 扩展集）交替 A/B，3 轮中位数：

| 样本 | 默认训练集 | 扩展训练集 | 变化 |
| --- | ---: | ---: | ---: |
| 留出 1v1（两个集都没训练） | 3.211s | 3.100s | -3.5% |
| 留出 2v2（两个集都没训练） | 3.016s | 3.001s | -0.5% |
| `aaa/bbb`（仅扩展集训练） | 2.353s | 2.316s | -1.6% |
| 复杂 1v1（仅扩展集训练） | 3.797s | 3.716s | -2.1% |
| fixed23（两个集都训练） | 3.088s | 3.118s | +1.0% |

结论：

- 新增训练样本对**新增样本自身**有约 -1.6%~-2.1% 的小幅收益，但留出样本
  -0.5%~-3.5%、已训练样本 ±1%，**全部在噪声带里**，方向也不完全一致；
- 训练样本量已经不是瓶颈：默认的 30 case + score 已经足够有代表性，
  **不建议为了这几个百分点去扩充并维护训练集**；
- 如果以后出现明显偏离现有 case 分布的新玩法（比如大量召唤/感染/多人局），
  再针对性地补那类输入即可，而不是把 benchmark 全量塞进去。
...
| `perf(core)` | 丢帧模式 `format!` 消息延迟构造 + 普通防御帧不走字符串比较 | ±1.5% 以内，工程清理 |
