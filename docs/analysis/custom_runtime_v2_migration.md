...
- `SummonExplode` effect 与 `push_summon_explode` helper 已覆盖 legacy 自爆的 `[0]使用[自爆]` replay、自爆者 HP 归零/移出 alive views、`get_at(true) * (4.0 + fire_mag)` 魔法伤害公式，以及目标受击 DIE/KILL 链后再执行自爆者 DIE；仍需后续补 dodge / pre_defend / post_defend / on_fire 等 `attacked()` 全链路 parity。
