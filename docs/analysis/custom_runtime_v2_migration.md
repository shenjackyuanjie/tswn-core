...
3. **minion fixture**：已覆盖 owner damage share 仍生效、minion heal 不向 owner 或 sibling minion 共享、owner death / explicit remove 清理 linked minion，以及 root owner entity slot 递增分配 `owner?N` minion 名称并按 legacy/custom 文案 spawn；后续补完整真实 minion handler。
...
- `next_minion_name_from_entity_slot` 已通过 root owner entity slot 记录 minion 名称计数，root owner 自身与 child minion 都按 `owner?N` 稳定分配；`push_minion_from_template_with_allocated_name` 已把名称分配、模板改名与 `SpawnWithMessage` 串起来，便于 shadow/zombie/summon-style minion handler 复用 legacy/custom 出场文案；`minion_display_index_for_entity` 已按 legacy `minion_display_index` 规则把 `?N` 转为 1-based 展示序号；child minion 缺少 `ReadAllies` capability 时返回结构化错误，避免跨实体读取绕过 capability。
