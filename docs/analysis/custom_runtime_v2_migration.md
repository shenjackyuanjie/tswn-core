...
1. **bed2 registry/import fixture**：已注册 `custom.bed2` kind、固定 summon skill、HP marker slot，并覆盖 `bed2[...]` / `@bed2` marker 到 v2 template 的最小导入、Player facade id-name 归一化桥接、typed summon template payload 读取后 spawn、grouped raw bed2 roster，以及 mixed legacy/bed2 raw roster 到 `PreparedCombatTemplate` 的 helper；后续补 runner 正式接入。
...
- bed2 / mixed roster 到 v2 runner 的正式接入。
