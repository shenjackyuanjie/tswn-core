...
- summon 完整内置技能迁移，完整 custom DIY/OL parser 与 CLI/wasm/Python/C API 切换接入（已有 parser-facing summon/shadow/zombie、组合 minion overlay 导入入口、默认 custom v2 profile、core `cli_api` custom v2 profile helper、C API 默认 normalized-run JSON 入口，以及 wasm 默认 normalized-run typed 入口），以及更多内置 minion handler 参数化/strict-diff parity。
