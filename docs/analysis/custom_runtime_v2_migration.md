...
- bed2-only 与 mixed roster import 已新增 parser-facing summon overlay 入口，可复用现有 `ol:` overlay parser，把首个 bed2 `ol.summon` 的 attrs、inherit_owner_def_res 和 `sklfire1` / `sklfire2` / `sklexplode` active order 转为 typed v2 `PlayerTemplate` payload，并通过 registry `export_name` 查找 v2 `SkillId` 后写入 template slot。
