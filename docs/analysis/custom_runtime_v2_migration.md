...
- `QueuedEffect::DisperseHit` 已覆盖 legacy `on_disperse` 命中后数据面：damage <= 0 跳过，damage > 0 时清目标 runtime/state positive、按 priority 输出取消消息，并按 `>64 => -64` / `>32 => 0` / `else -32` 规则扣目标 MP；完整 Disperse 主动技能的目标选择与魔法攻击链仍需后续接入。
