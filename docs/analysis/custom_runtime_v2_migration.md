...
- `SummonExplode` effect 与 `push_summon_explode` helper 已覆盖 legacy 自爆的 `[0]使用[自爆]` replay、自爆者 HP 归零/移出 alive views、目标伤害，以及目标受击 DIE/KILL 链后再执行自爆者 DIE；仍需后续补 `get_at(true, rng)` 与 fire_mag 倍率 parity。
...
- `PlayerTemplate` / `PlayerRuntime` 已承载 magic 与 at_boost_millionths，`PlayerRuntime::get_at` 已复刻 legacy `get_at(true/false)` 的 RNG 取值公式；后续可把 summon 自爆 amount 接到 `get_at(true) * (4.0 + fire_mag)`。
