...
- `SummonExplode` 已接入 target-side `PRE_DEFEND` / `POST_DEFEND` skill/state hook，context 可暴露并改写当前攻击量或最终伤害；`StatePayload::ShieldValue` 与 `run_shield_post_defend_state` 已覆盖 ShieldState 护盾吸收/耗尽数据面；
