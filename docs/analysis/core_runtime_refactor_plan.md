...
- 已从 custom large / fight_multi 真实 raw 输入抽出初始化 parity golden，覆盖 v2 raw runner 对 legacy seed RNG、team 编号与 WorldArena 初始派生视图的对齐；large 与 fight_multi 真实 raw 已补前 3 轮 normalized prefix golden，固定 RNG checkpoint、HP/MP/防御/魔防、round/alive 派生视图、action/frame 与 guard 状态；
...
- `run_legacy_summon_recast_from_template_slot_with_config`、`run_shadow_minion_from_template_slot_with_config`、`run_zombie_minion_from_template_slot_with_config` 已把 summon/shadow/zombie fixture 闭包提升为可配置正式 handler，复用 typed template slot、entity slot counter 与 legacy replay 顺序；
...
- damage/heal/state/spawn/revive/remove/replay/custom/summon-explode effect 接入；
...
- `CustomBed2Import` / `RuntimeV2Runner` 已补 parser-facing summon/shadow/zombie overlay 导入入口、组合 minion overlay 导入入口和 `CustomRuntimeV2ImportConfig` profile 入口，可从 bed2-only 与 mixed roster/runner 的 bed2 raw `ol.summon` 解析 attrs、inherit_owner_def_res 与 summon fire/explode active order，并可从 `ol.shadow` 解析 attrs 与 possess active order、从 `ol.zombie` 解析 attrs 与 skill export 前缀映射，把 typed `PlayerTemplate` payload 写入 v2 template slot；core 已新增 `default_custom_runtime_v2_import_config`、显式 profile 与默认 profile 两组 `custom_runtime_v2_*` / `default_custom_runtime_v2_*` 专用外层 helper，CLI 已暴露 `runtime-v2 normalized-run` JSON 命令并补结构化 JSON golden 覆盖 rounds / RNG / entity stats / action / frame / `update_type` 字段形状，C API 已暴露 `tswn_default_custom_runtime_v2_normalized_run_json` 并补结构化 JSON golden 固定 rounds / RNG / entity stats / action / frame / `update_type` 字段形状，Python 已暴露 `default_custom_runtime_v2_normalized_run` dict 入口并补 dict golden 固定 rounds / RNG / entity stats / action / frame 与 `update_type` 字段形状，wasm 已暴露 `default_custom_runtime_v2_normalized_run` typed 入口并补 typed view golden 固定 rounds / RNG / entity stats / action / frame / `UpdateTypeView` 字段形状，作为默认 custom v2 normalized run 绑定入口，先提供 custom profile raw import 与 normalized run 调用面，不替换现有 legacy API；
...
- 已补 `show-wasm.test.mjs` 纯 JS adapter 与 HTML chunk 测试，固定 v2 normalized run 到 show-compatible players / states / rows / clips / sequential HP bar / recover HP bar / multi-target sidebar / winner row / summoned entity first-appearance shape 的转换和 `buildFrameRows()` 渲染；
