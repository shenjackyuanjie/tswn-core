...
- 已从 custom large / fight_multi 真实 raw 输入抽出初始化 parity golden，覆盖 v2 raw runner 对 legacy seed RNG、team 编号与 WorldArena 初始派生视图的对齐；large 与 fight_multi 真实 raw 已补前 4 轮 normalized prefix golden，固定 RNG checkpoint、HP/MP/防御/魔防、round/alive 派生视图、action/frame 与 guard 状态；
