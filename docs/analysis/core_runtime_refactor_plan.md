...
- 已补 `show-wasm.test.mjs` 纯 JS adapter 与 HTML chunk 测试，固定 v2 normalized run 到 show-compatible players / states / rows / clips / HP bar / winner row shape 的转换和 `buildFrameRows()` 渲染；
