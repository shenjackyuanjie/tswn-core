...
- `CustomBed2Import` 已补 parser-facing summon overlay 导入入口，可从 bed2 raw `ol.summon` 解析 attrs、inherit_owner_def_res 与 summon fire/explode active order，并把 typed `PlayerTemplate` payload 写入 v2 template slot；
