...
| 容量 profile | `baseline-64`（已冻结）；变更须发布新 profile | **已冻结 `E_max=64`**，`E/T/R/H/L/S/Q/V/X = 64/32/32/512/4096/64/512/32768/65536`，逐维推导见第 4 节容量表。实测依据：100k 池（2v2v2）`e≤30`、`h≤120`、`l≤882`、`q≤96`、`V≤2800`、`X≤2271`；8 人池全部 20 种队伍划分最高 `e=44`、`h=176`、`l=1481`、`q=140`、`V=4630`、`X=3335`；3v2v2v1 深测到 `e=46`、`h=184`、`l=1560`、`q=146`、`V=4846`、`X=3489`。实体数机制审计（第 4 节）确认不存在与输入无关的上界，所以 `E_max` 是**支持域选择**：8 人以内、召唤密度不超过该池。配套：`stats` 报每维溢出率，超限返回 `CapacityExceeded` 而不截断；`Q_max`／`S_max` 随 `E_max` 同步提到 512／64，`H_max` 按实测 `h≈4e` 定 512，`X_max` 每槽按第 3.2 节白名单 3 条计费（原 66 条/槽在冻结白名单下不可达）。要机制保证只能改引擎（给 shadow／clone 加记忆槽或加全局上限），会改变战斗语义，本轮不做 |
