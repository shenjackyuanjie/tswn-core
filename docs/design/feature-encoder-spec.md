...
| --- | --- | --- |
| `E_max`（实体） | 32（**已被 8 人池实测打穿，见下文**） | `E_max ≥ e`。100k 池（2v2v2）实测 max=30，据此取 `ceil_pow2(30)=32`（P:161 的 p99=15、max=30）；但 8 人固定池的 FFA 6／7／8 实测 `e` 为 37／38／40，支持域若含 8 人 FFA 则须提到 64。本轮不擅自改值，见第 16 节 |
...
1. `E_max=32` 不足：FFA 6／7／8 的 `e` 峰值 37／38／40，按第 4 节的 `CapacityExceeded` 规则这些样本会被拒。支持域若含 8 人 FFA，`E_max` 至少要 64（`ceil_pow2(40)`）。
2. “按实测收紧”的候选值同样不成立：本池 `l=1429>1024`、`V=4444>4096`，`q=128` 正好顶格候选的 `Q_max=128`。本表的 `H_max=512`、`L_max=4096`、`Q_max=256`、`V_max=32768`、`X_max=65536` 在该池下仍有余量。
3. 峰值随玩家数近似线性（`h≈4e`、`l≈36e`、`V≈111e`、`X≈76e`），但“每玩家实体数”由名字决定的 build 强烈影响：同一批 8 人在 3684 人名字池的 FFA8 里只有 24 个实体，本池到 40。**不能按单一名字池的分布收紧容量。**
4. 因此第 16 节“容量 profile”的冻结条件仍未满足：要么把 `E_max` 提到 64 并同步复核 `H_max`／`L_max`／`V_max`／`X_max`，要么先给出支持域内实体数的机制上界（例如每玩家召唤上限）再定 `E_max`。`baseline-32` 名字里的 32 已不再与实测自洽。
...
| 容量 profile | baseline-32；或补测后单独推出更大 profile | **已实测两批峰值（第 4 节两张表）**：100k 池（2v2v2）`e≤30`、`h≤120`、`l≤882`、`q≤96`、`V≤2800`、`X≤2271`；8 人固定池跨 1v1／2v2／2v2v2／FFA 3–8 后 `e` 最高 40、`h` 160、`l` 1429、`q` 128、`V` 4444、`X` 3032。结论：`E_max=32` 已不足以覆盖 8 人 FFA，且“按实测收紧”候选（`L_max=1024`、`V_max=4096`、`Q_max=128`）同样被打穿。**冻结条件**：先定 `E_max`（64，或先给出支持域内实体数的机制上界），再同步复核 `H_max`／`L_max`／`V_max`／`X_max` 并重算足迹；不使用静默截断 |
