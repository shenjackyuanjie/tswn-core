...
除按阵容哈希切分的 matchup-disjoint 训练/验证/测试外，还需要第二套 **actor/name-disjoint 评估**：让部分角色的机制指纹只出现在测试集合，用来区分“模型从局面泛化”与“模型记住角色机制”。这不是数据泄漏修复，而是泛化能力评估；它要求生成侧在 audit metadata 里额外记录输入行的角色标识（该标识不得进入 `state`），评估侧据此分组。当前数据集与 `stats` 还没有这一维度。

模型规模与指标：第一版控制在 50K–150K 参数、优先实体/队伍聚合结构（DeepSets 形态）。评估同时报告 Log Loss、Brier 与 ECE／校准曲线，`progress` 只用于分桶分析、不能进入特征。部署优先 Rust 手写 inference（WASM 体积小、启动快、数值容易对齐），在结构稳定前不引入 ONNX Runtime／Candle／Burn／tract。
