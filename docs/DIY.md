...
普通角色和 DIY 角色的 clone 构建现在使用统一的 "先 clamp 再 boost" 路径（对齐 JS 衰减下限语义）：

1. **检测 boost 候选**：从 name_base 推导的技能等级中，识别哪些技能吃到 `boost_last`（末尾主动技翻倍）或 `boost_level`（末尾座位加成）。boost 元数据记录到 `Skill.diy_boost`。
2. **clamp 到 owner**：将 clone 的技能等级截断到 owner 当前（已衰减）等级。
3. **重新执行 SkillBoost 加成**：对 clamp 后的等级重新执行翻倍或加值，加成金额直接由 `diy_boost` 元数据决定：
   - `Normal`：不处理，保持 clamp 后等级
   - `LastBoost`：衰减后等级 × 2（翻倍）
   - `SlotBoost`：衰减后等级 + boost（执行 +b）

对于 DIY 角色，boost 元数据来自 overlay 配置；对于普通角色，boost 元数据在 build 阶段由 `boost_if_not()` / `boost_level()` 自动记录。
