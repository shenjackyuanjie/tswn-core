...
impl SkillTrait for CounterSkill {
    ...
    fn on_update_end_with_level(&mut self, _level: u32, args: SkillArgs) -> bool {
        ...
        let core = {
            let target_plr = args.3.just_get_player_mut(target).expect("cannot get counter target from storage");
        ...
        if core.hit {
            ...
            let target_plr = args.3.just_get_player_mut(core.target).expect("cannot get counter target from storage");
