...
fn player_raw_new_parses_diy_overlay() {
    ...
    assert_eq!(
        skills.iter().find(|(k, _)| k == "sklfire").map(|(_, v)| v),
        Some(&SkillBoost::Normal(5))
    );
    assert_eq!(
        skills.iter().find(|(k, _)| k == "reflect").map(|(_, v)| v),
        Some(&SkillBoost::Normal(2))
    );
    ...
    assert_eq!(slot_boost, Some(&SkillBoost::SlotBoost { base: 40, boost: 30 }));
    ...
    assert_eq!(last_boost, Some(&SkillBoost::LastBoost(46)));
    ...
    let player = Player::new_from_namerena_raw(
        "minion+ol:{\"attrs\":[86,86,86,86,86,86,86,300],\"shadow\":{\"attrs\":[46,47,48,49,50,51,52,200],\"skills\":{\"sklpossess\":9}},\"summon\":{\"skills\":{\"sklfire1\":12,\"sklexplode\":3}},\"zombie\":{\"attrs\":[40,41,42,43,44,45,46,90],\"skills\":{\"sklrapid\":7}}}".to_string(),
        storage.clone(),
    )
    .unwrap();
    let overlay = player.overlay.as_ref().expect("should parse overlay");
...
#[test]
fn ol_overlay_with_summon_skill_map_keeps_main_attrs() {
    let storage = Storage::new_arc();
    let mut player = Player::new_from_namerena_raw(
        "milk@lzk+ol:{\"attrs\":[95,86,94,89,85,89,87,343],\"skills\":{\"sklexchange\":14,\"sklclone\":22,\"sklsummon\":10,\"sklcounter\":6,\"sklslow\":\"2*23\",\"sklreraise\":\"41+36\"},\"name_factor_enabled\":true,\"summon\":{\"attrs\":[36,86,56,55,36,89,88,89],\"skills\":{\"sklexplode\":0,\"sklfire2\":4,\"sklfire1\":\"2*14\"},\"reuse_skills_on_recast\":true}}".to_string(),
        storage,
    )
    .unwrap();
...
fn bed2_marker_parses_as_type_and_body_overlay() {
    let storage = Storage::new_arc();
    let mut player = Player::new_from_namerena_raw("alpha@red+bed2[1234]".to_string(), storage).unwrap();

    assert_eq!(player.name, "alpha");
    assert_eq!(player.team, Some("red".to_string()));
    assert_eq!(player.player_type(), PlayerType::Bed2);
    assert_eq!(player.clan_name(), "red");
    assert_eq!(player.id_key_name(), "alpha@red+bed2[1234]");
    assert_eq!(player.display_name(), "被子");
    assert_eq!(Player::raw_namerena_to_idname("alpha@red+bed2[1234]"), "alpha@red+bed2[1234]");
    assert_eq!(Player::raw_namerena_to_idname("alpha@red@bed2"), "alpha@red+bed2[3000]");

    player.build();
    assert_eq!(player.attr, [0, 99, 0, 0, 0, 99, 0, 1234]);
    assert_eq!(player.get_status().max_hp, 1234);
...
fn diy_overlay_name_factor_disabled_forces_zero() {
    ...
    let player = Player::new_and_init_with_overlay(None, "aaaaa".to_string(), None, Some(overlay), storage.clone()).unwrap();
