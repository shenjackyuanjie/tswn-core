...
impl CombatRuntime {
    ...
    pub fn score_plain_charm_target(&mut self, target: EntityIdx, smart: bool) -> f64 {
        let entity = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("unknown runtime charm target: {}", target.0));
    ...
    pub fn drain_plain_charm_skill_into(&mut self, actor: EntityIdx, target: EntityIdx, updates: &mut RunUpdates) {
        ...
        let (owner_magic, charge_active, caster_effective_team_idx) = {
            let owner = self
                .entities
                .get(actor)
                .unwrap_or_else(|| panic!("unknown runtime charm actor: {}", actor.0));
            (
                owner.runtime.magic,
        ...
        if let Some((mut group_id, effective_team_idx, mut source_team_idx, state_target, mut step)) = existing {
            ...
            assert!(
                ...
                "runtime charm state disappeared during recharm"
            );
        } else {
            let charm_state_id = self
                .registry
                .state_id_by_export_name(DEFAULT_CORE_CHARM_STATE_EXPORT)
                .expect("default runtime profile must register core charm state");
            let charm_priority = self
                .registry
                .state(charm_state_id)
                .expect("default runtime core charm state disappeared")
                .priority;
            assert!(
                ...
                "runtime charm state should be inserted"
            );
    ...
    pub fn select_plain_heal_targets(&mut self, actor: EntityIdx, smart: bool) -> PreparedTargetList {
        ...
        if probe_heal {
            let described = candidates
                .iter()
                .map(|target| {
                    let entity = self.entities.get(*target).expect("runtime heal candidate should exist");
                    format!(
                        "{}:{}:{}/{}",
                ...
                .collect::<Vec<_>>();
            eprintln!(
                "[heal_select:runtime:start] actor={} smart={} candidates={:?} rc4=({}, {})",
        ...
        while dup <= select_count && invalid <= select_count as i32 {
            ...
            if probe_heal {
                eprintln!(
                    "[heal_select:runtime:pick] target={} valid={} dup={} invalid={} rc4=({}, {}) -> ({}, {})",
        ...
        for target in selected {
            ...
            if probe_heal {
                eprintln!(
                    "[heal_select:runtime:score] target={} score={} rc4=({}, {})",
        ...
        if probe_heal {
            eprintln!(
                "[heal_select:runtime:done] scored={:?} rc4=({}, {})",
                scored, self.rng.i, self.rng.j
            );
        }
    ...
    pub fn score_plain_heal_target(&mut self, target: EntityIdx, smart: bool) -> f64 {
        ...
        let entity = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("unknown runtime heal target: {}", target.0));
    ...
    pub fn drain_plain_heal_skill_into(
        ...
        let current_level = self
            ...
            .unwrap_or_else(|| panic!("runtime heal level missing for fixed lane {fixed_lane}"));
        let atp = self
            .entities
            .get(actor)
            .unwrap_or_else(|| panic!("unknown runtime heal actor: {}", actor.0))
            .runtime
        ...
        if crate::debug::debug_heal() {
            let owner = self.entities.get(actor).expect("runtime heal owner should exist");
            let target_entity = self.entities.get(target).expect("runtime heal target should exist");
            eprintln!(
                "[heal:runtime] owner={} target={} owner_hp={}/{} target_hp={}/{} at_boost={} atp={:.2} missing={} heal={} rc4=({}, {})",
                owner.template.name,
        ...
        assert!(
            ...
            "runtime heal fixed lane disappeared during action"
        );
    }
