...
impl CombatRuntime {
    pub fn apply_disperse_hit_into(&mut self, caster: EntityIdx, target: EntityIdx, updates: &mut RunUpdates) {
        let Some(target_entity) = self.entities.get_mut(target) else {
            panic!("unknown runtime disperse target entity: {}", target.0);
        };
    ...
    pub fn has_alive_enemy_or_pending_spawn(&self, caster: EntityIdx) -> bool {
        let caster_team = self
            .entities
            .get(caster)
            .unwrap_or_else(|| panic!("runtime kill caster disappeared: {}", caster.0))
            .runtime
            .team;
        if self
            .world
    ...
    pub fn drain_plain_protect_pre_defend_into(
        ...
        let is_magic = defend_value.is_magic().expect("runtime protect PRE_DEFEND value must carry attack type");
        let caster = defend_value.caster();
        let target_team = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("runtime protect target disappeared: {}", target.0))
            .runtime
            .team;
        ...
        loop {
            ...
            let link = self
                ...
                .get(link_index)
                .cloned()
                .unwrap_or_else(|| panic!("runtime protect link index disappeared: {link_index}"));
            ...
            if trigger_ok && protector_ready {
                ...
                let redirected_atp = redirected_atp.atp().expect("runtime protect pre-defend hooks must leave an atp value");
                ...
                let defense = {
                    let protector = self
                        .entities
                        .get(link.owner)
                        .unwrap_or_else(|| panic!("runtime protector disappeared: {}", link.owner.0));
                    if is_magic {
                ...
                let redirected_damage = redirected_damage_value
                    .damage()
                    .expect("runtime protect post-defend hooks must leave a damage value");
        ...
        loop {
            ...
            let (skill_generation, state_generation, shield_active) = {
                let entity = self
                    .entities
                    .get(target)
                    .unwrap_or_else(|| panic!("runtime 防御钩子目标已消失：{}", target.0));
                (
    ...
    pub fn apply_runtime_shield_post_defend(&mut self, target: EntityIdx, defend_value: &mut RuntimeDefendValue) {
        ...
        let target = self
            .entities
            .get_mut(target)
            .unwrap_or_else(|| panic!("runtime shield target disappeared: {}", target.0));
    ...
    pub fn apply_plain_legacy_damage_into(
        ...
        let target_entity = self
            .entities
            .get_mut(target)
            .unwrap_or_else(|| panic!("unknown runtime plain legacy damage target entity: {}", target.0));
    ...
    pub fn apply_poison_tick_damage_into(
        ...
        let target_entity = self
            .entities
            .get_mut(target)
            .unwrap_or_else(|| panic!("unknown runtime poison target entity: {}", target.0));
        target_entity.runtime.hp = (target_entity.runtime.hp - amount).max(0);
        let was_alive = target_entity.runtime.alive;
        updates.add(RuntimeFrame::legacy_damage_update(caster.0 as usize, target.0 as usize, amount));
        self.drain_plain_post_damage_skill_chain_into(target, amount, caster, updates);
    ...
    pub fn apply_disperse_attack_damage_into(
        ...
        let Some(target_entity) = self.entities.get_mut(target) else {
            panic!("unknown runtime disperse damage target entity: {}", target.0);
        };
        target_entity.runtime.hp = (target_entity.runtime.hp - amount).max(0);
        let was_alive = target_entity.runtime.alive;
        updates.add(RuntimeFrame::legacy_damage_update(caster.0 as usize, target.0 as usize, amount));
    ...
    pub fn emit_poison_release_if_cleared(&mut self, target: EntityIdx, updates: &mut RunUpdates) {
        let Some(target_entity) = self.entities.get(target) else {
            panic!("unknown runtime poison release target entity: {}", target.0);
        };
        if target_entity
            .states
            .entries()
    ...
    pub fn magic_attack_dodged(&mut self, caster: EntityIdx, target: EntityIdx) -> bool {
        let Some(target_entity) = self.entities.get(target) else {
            panic!("unknown runtime magic attack dodge target entity: {}", target.0);
        };
    ...
    pub fn apply_fire_on_damage(&mut self, target: EntityIdx, fire_state_key: u32) {
        let Some(target_entity) = self.entities.get(target) else {
            panic!("unknown runtime fire target entity: {}", target.0);
        };
    ...
    pub fn apply_ice_on_damage(&mut self, caster: EntityIdx, target: EntityIdx, updates: &mut RunUpdates) {
        let Some(target_entity) = self.entities.get(target) else {
            panic!("unknown runtime ice target entity: {}", target.0);
        };
    ...
    pub fn kill_entity_without_damage_into(&mut self, target: EntityIdx, updates: &mut RunUpdates) -> bool {
        ...
        if killed {
            let team = self
                .entities
                .get(target)
                .unwrap_or_else(|| panic!("unknown runtime self-death target entity: {}", target.0))
                .runtime
                .team;
    ...
    pub fn kill_entity_without_damage_mark_only_into(&mut self, target: EntityIdx) -> bool {
        let Some(target_entity) = self.entities.get_mut(target) else {
            panic!("unknown runtime self-death target entity: {}", target.0);
        };
    ...
    pub fn cleanup_linked_minions_for_owner_except_if(
        ...
        for minion in linked_minions {
            let Some(minion_entity) = self.entities.get_mut(minion) else {
                panic!("unknown runtime linked minion entity: {}", minion.0);
            };
    ...
    pub fn resolve_damage_target(&self, target: EntityIdx) -> EntityIdx {
        let target_entity = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("unknown runtime damage target entity: {}", target.0));
    ...
    pub fn ensure_effect_entity(&self, effect: &'static str, role: &'static str, entity: EntityIdx) {
        if self.entities.get(entity).is_none() {
            panic!("unknown runtime {effect} {role} entity: {}", entity.0);
        }
    }
}
