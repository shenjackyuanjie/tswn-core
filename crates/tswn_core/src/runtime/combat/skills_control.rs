...
impl CombatRuntime {
    pub fn drain_plain_half_skill_into(&mut self, actor: EntityIdx, target: EntityIdx, updates: &mut RunUpdates) {
        ...
        let (owner_wisdom, owner_magic, charge_active) = {
            let owner = self
                .entities
                .get(actor)
                .unwrap_or_else(|| panic!("unknown runtime half actor: {}", actor.0));
            (
        ...
        let (target_hp, target_resistance, target_agility, target_flags, target_name, target_active) = {
            let target_entity = self
                .entities
                .get(target)
                .unwrap_or_else(|| panic!("unknown runtime half target: {}", target.0));
            (
        ...
        self.entities
            .get_mut(target)
            .unwrap_or_else(|| panic!("runtime half target disappeared: {}", target.0))
            .runtime
            .hp = new_hp;
    ...
    pub fn score_plain_ice_target(&mut self, target: EntityIdx, smart: bool) -> f64 {
        let entity = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("unknown runtime ice target: {}", target.0));
    ...
    pub fn drain_plain_accumulate_skill_into(&mut self, actor: EntityIdx, updates: &mut RunUpdates) {
        let owner = self
            .entities
            .get_mut(actor)
            .unwrap_or_else(|| panic!("unknown runtime accumulate owner: {}", actor.0));
    ...
    pub fn drain_plain_clone_skill_into(&mut self, actor: EntityIdx, fixed_lane: usize, updates: &mut RunUpdates) {
        ...
        let clone_kind = self
            ...
            .expect("default runtime profile must register core clone kind");
        ...
        if !charge_active {
            let owner = self
                .entities
                .get_mut(actor)
                .unwrap_or_else(|| panic!("unknown runtime clone actor: {}", actor.0));
        ...
        let (
            ...
            let root_name = self
                .entities
                .get(root_owner)
                .unwrap_or_else(|| panic!("unknown runtime clone root owner: {}", root_owner.0))
                .template
                .name
                .clone();
            ...
            let zombie_blueprint = zombie_blueprint_slot.and_then(|slot| match owner.slots.get(slot) {
                Some(SlotValue::PlayerTemplate(template)) => Some(template.as_ref().clone()),
                Some(_) => panic!("runtime core zombie blueprint slot has invalid value"),
                None => None,
            });
            (
                root_owner,
        ...
        assert!(
            ...
            "runtime clone fixed lane disappeared while building child"
        );
        ...
        assert!(
            ...
            "runtime clone fixed lane disappeared while updating owner"
        );
        ...
        let clone_entity = self
            .entities
            .get_mut(clone_idx)
            .unwrap_or_else(|| panic!("runtime clone spawn missing entity {}", clone_idx.0));
    ...
    pub fn select_plain_exchange_targets(&mut self, actor: EntityIdx, smart: bool) -> PreparedTargetList {
        let actor_hp = self
            ...
            .unwrap_or_else(|| panic!("unknown runtime exchange actor: {}", actor.0));
        let actor_team = self.plain_effective_team(actor);
        let all_alive = self.world.flat_alive();
        if all_alive.is_empty() {
            return PreparedTargetList::new();
    ...
    pub fn drain_plain_exchange_skill_into(
        ...
        let current_level = self
            ...
            .get(actor)
            .and_then(|entity| entity.template.skills.level_at(fixed_lane))
            .unwrap_or_else(|| panic!("runtime exchange level missing for fixed lane {fixed_lane}"));
        assert!(
            self.entities
                .get_mut(actor)
                .unwrap()
        ...
        if std::env::var_os("TSWN_PROBE_EXCHANGE").is_some() {
            ...
            eprintln!(
                "[exchange_probe:runtime:before] round={} owner={} target={} owner_hp={} target_hp={} owner_max_hp={} \
        ...
        if std::env::var_os("TSWN_PROBE_EXCHANGE").is_some() {
            ...
            eprintln!(
                "[exchange_probe:runtime:after] round={} owner_hp={} target_hp={} owner_move={} target_move={} rc4=({}, {})",
                self.round + 1,
                owner.runtime.hp,
                target_entity.runtime.hp,
                owner.runtime.move_state.speed_points,
