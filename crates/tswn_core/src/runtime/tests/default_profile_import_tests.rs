...
#[test]
fn root_runner_builds_the_main_runtime_with_the_default_profile() {
    let mut runner = crate::Runner::new_from_namerena_raw("left@red\n\nright@blue".to_owned())
        .expect("root Runner should build the main runtime");

    assert_eq!(runner.runtime().entities.len(), 2);
    assert!(runner.run_round().action.is_some());
}

#[test]
fn runtime_custom_import_profile_builds_mixed_raw_with_minion_overlays() {
    ...
    let config = CustomRuntimeImportConfig::new(registry, bed2, summon)
...
#[test]
fn runtime_custom_import_profile_wraps_missing_overlay_skill_errors() {
    let mut builder = ExtensionRegistryBuilder::default();
    let summon = builder
        .register_skill("custom", "summon", "custom.summon", TargetPolicy::Enemy, SkillPriority(0))
        .expect("summon skill should register");
    ...
    let registry = builder.build();
    let config =
        CustomRuntimeImportConfig::new(registry, bed2, summon).with_bed2_minion_overlays(CustomBed2MinionOverlayConfig {
            summon: CustomBed2SummonTemplateConfig {
                template_slot: summon_template_slot,
                summon_kind,
                fire_skill_export_name: "custom.summon.fire",
    ...
    let err = RuntimeRunner::from_custom_mixed_namerena_raw(
    ...
    assert_eq!(
        err,
        CustomRuntimeImportError::Bed2MinionOverlay(CustomBed2MinionOverlayImportError::Shadow(
...
#[test]
fn default_custom_runtime_profile_builds_mixed_raw_runner() {
...
fn default_profile_imports_plain_defend_skill_level_from_legacy_loadout() {
    ...
    let raw = "left@red\n\nright@blue\n";
    let runner =
        RuntimeRunner::from_custom_mixed_namerena_raw(raw.to_owned(), config).expect("plain raw should construct runtime runner");
    let legacy = crate::LegacyRunner::new_from_namerena_raw(raw.to_owned()).expect("plain raw should construct legacy runner");
    let snapshot = legacy
        .storage
        .get_player(&1)
    ...
    let right = runner.runtime().entities.get(EntityIdx(1)).expect("right runtime player should exist");
    ...
    let defend_lane = right
        ...
        .unwrap_or_else(|| panic!("runtime loadout should contain DefendSkill; legacy snapshot: {snapshot:?}"));
...
fn default_profile_imports_plain_merge_kill_hook_from_legacy_loadout() {
    ...
    let legacy = crate::LegacyRunner::new_from_namerena_raw(raw.to_owned()).expect("large_51 raw should construct legacy runner");
    let snapshot = legacy
        .storage
        .get_player(&0)
    ...
    let runner = RuntimeRunner::from_custom_mixed_namerena_raw(raw.to_owned(), config)
        .expect("large_51 raw should construct runtime runner");
    let owner = runner.runtime().entities.get(EntityIdx(0)).expect("large_51 runtime owner should exist");
    let merge_lane = owner
        ...
        .unwrap_or_else(|| panic!("runtime loadout should contain MergeSkill; legacy snapshot: {snapshot:?}"));
...
fn default_profile_imports_and_executes_plain_shadow_blueprint() {
    ...
    let mut runner = RuntimeRunner::from_custom_mixed_namerena_raw(raw.to_owned(), config)
        .expect("large_51 raw should construct runtime runner");
    let owner = runner.runtime().entities.get(EntityIdx(0)).expect("runtime shadow owner should exist");
    let shadow_lane = owner
        ...
        .expect("runtime owner should import ShadowSkill");
    assert_eq!(owner.template.skills.level_at(shadow_lane), Some(expected_level));
    let SlotValue::PlayerTemplate(blueprint) = owner
        .slots
        .get(blueprint_slot)
        .expect("runtime owner should store a per-owner shadow blueprint")
    else {
        panic!("runtime shadow blueprint slot should hold PlayerTemplate");
    };
    ...
    let possess_lane = blueprint
        ...
        .expect("runtime shadow blueprint should import PossessSkill");
