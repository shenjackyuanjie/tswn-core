...
#[test]
fn runtime_runner_runs_mixed_namerena_raw_fixture_shape() {
    ...
    let mut runner = prepared_init_tests::runtime_runner_from_raw(raw_input, |raw_groups| {
        RuntimeRunner::from_mixed_roster(raw_groups, registry, bed2, summon)
    });
    ...
    let initial_rng = crate::runtime::oracle::NormalizedRngCheckpoint::from_runtime(runner.runtime());
...
#[test]
fn runtime_runner_rejects_plain_rows_in_bed2_roster() {
    let mut builder = ExtensionRegistryBuilder::default();
    let summon = builder
        .register_skill("custom", "summon", "custom.summon", TargetPolicy::Enemy, SkillPriority(0))
        .expect("summon skill should register");
    ...
    let err = RuntimeRunner::from_bed2_roster(&raw_groups, registry, bed2, summon)
...
#[test]
fn runtime_runner_runs_mixed_roster_until_winner() {
    let mut builder = ExtensionRegistryBuilder::default();
    let summon = builder
        .register_skill("custom", "summon", "custom.summon", TargetPolicy::Enemy, SkillPriority(0))
        .expect("summon skill should register");
