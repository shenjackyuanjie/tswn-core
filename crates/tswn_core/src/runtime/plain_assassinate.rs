...
impl CombatRuntime {
    pub fn run_plain_skill_pre_action_accumulator(&mut self, actor: EntityIdx) -> PlainSkillPreActionOutcome {
        let pre_action_order = self
            .entities
            .get(actor)
            .unwrap_or_else(|| panic!("unknown runtime pre-action owner: {}", actor.0))
            .template
            .skills
            .pre_action_order()
            .iter()
        ...
        for fixed_lane in pre_action_order {
            let skill_id = self
                ...
                .copied()
                .unwrap_or_else(|| panic!("runtime pre-action order references missing fixed lane {fixed_lane}"));
            let export_name = self
                .registry
                .skill(skill_id)
                .unwrap_or_else(|| panic!("unknown runtime pre-action skill id: {}", skill_id.0))
                .export_name
                .as_str();
            match export_name {
                ...
                export_name if export_name == BuiltinActiveSkill::Assassinate.export_name() => {
                    ...
                    let Some(pending) = pending else {
                        self.entities
                            .get_mut(actor)
                            .expect("runtime assassinate owner disappeared")
                            .template
                            .skills
    ...
    fn score_plain_assassinate_target(&mut self, target: EntityIdx, smart: bool) -> f64 {
        let entity = self
            .entities
            .get(target)
            .unwrap_or_else(|| panic!("unknown runtime assassinate target: {}", target.0));
        if !smart {
    ...
    fn clear_plain_assassinate_pending(&mut self, actor: EntityIdx) -> Option<AssassinateRuntime> {
        let entity = self
            .entities
            .get_mut(actor)
            .unwrap_or_else(|| panic!("unknown runtime assassinate owner: {}", actor.0));
        ...
        if let Some(pending) = self.clear_plain_assassinate_pending(actor) {
            ...
            let atp = {
                let owner = &self
                    .entities
                    .get(actor)
                    .unwrap_or_else(|| panic!("runtime assassinate owner disappeared: {}", actor.0))
                    .runtime;
    ...
    pub(super) fn run_plain_assassinate_post_damage_into(&mut self, owner: EntityIdx, damage: i32, updates: &mut RunUpdates) {
        ...
        let pending = self
            ...
            .expect("runtime assassinate pending disappeared during post-damage");
        updates.add_newline();
        updates.add(RuntimeFrame::replay_update(
            owner.0 as usize,
