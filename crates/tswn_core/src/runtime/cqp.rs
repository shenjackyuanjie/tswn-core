...
use super::{
    ...
    default_custom_runtime_import_config, prepared_runtime_win_rate,
};
...
struct MatchupEvent {
    index: usize,
    result: RuntimeCqpMatchupResult,
}
...
fn run_matchup(
    matchup: &RuntimeCqpMatchup,
    n: usize,
    eval_rq: f64,
    config: &CustomRuntimeImportConfig<'static>,
) -> RuntimeCqpMatchupResult {
    let started = Instant::now();
    let summary = (|| {
        let prepared = PreparedRuntimeRunner::from_custom_mixed_roster_with_eval_rq(&matchup.groups, eval_rq, config.clone())?;
        prepared_runtime_win_rate(&prepared, n, 1)
    })();
    RuntimeCqpMatchupResult {
        summary,
