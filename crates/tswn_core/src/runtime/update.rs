...
impl RunUpdate {
    ...
    /// 从语言包键创建一条事件消息帧。
    ///
    /// 内部调用 [`crate::runtime::lang::get_lang`] 查找对应的中文模板字符串，
    /// 再转交 [`RunUpdate::new`] 构造。若键不存在，消息为空字符串。
    ///
    /// # 参数
    /// - `key`：语言包键名，建议使用 [`crate::runtime::lang::keys`] 中定义的常量。
    /// - `caster`：施法者 ID。
    /// - `target`：目标 ID。
    /// - `score`：UI 显示分值。
    ///
    ...
    ///
    /// ```rust,ignore
    /// use tswn_core::runtime::update::RunUpdate;
    /// use tswn_core::runtime::lang::keys;
    ///
    ...
    pub fn new_lang(key: &str, caster: PlrId, target: PlrId, score: u32) -> Self {
        Self::new(crate::runtime::lang::get_lang(key), caster, target, score)
    }
