//! `namer-pf` 技能榜。
//!
//! 把待评名字组导出为 `+diy`，取全组熟练度最高的技能，按阈值配置筛选输出。
//! 逻辑与 tswn_openbox GUI 的 `backend/skill_board.rs` 逐条对齐：
//! - 取全组最高等级技能（等级 > 0），同等级多个技能按 key 排序去重；
//! - 全组技能等级全部小于 30（或没有可导出技能）时，额外按 `[lessskl]` 白板号阈值筛选；
//! - `全能` 除 `all` 阈值外还有硬编码的四项下限；
//! - 技能等级支持 `"40+30"` 相加与 `"2*22"` 相乘两种字符串写法。
...
/// 技能榜阈值配置：技能 key -> 各指标阈值。
#[derive(Debug, Clone)]
pub(super) struct SkillBoardConfig {
    entries: HashMap<String, SkillBoardThreshold>,
}

/// 单个技能的阈值配置；未配置的指标不参与筛选。
#[derive(Debug, Clone, Copy, Deserialize)]
pub(super) struct SkillBoardThreshold {
    pub pp: Option<u64>,
    pub pd: Option<u64>,
    pub qp: Option<u64>,
    pub qd: Option<u64>,
...
/// 一条技能榜输出行。
#[derive(Debug, Clone)]
pub(super) struct SkillBoardLine {
    pub title: String,
    pub score: f64,
}
...
/// 对单个名字组计算技能榜输出行。
pub(super) fn evaluate_skill_board(
    name_group: &[String],
...
mod tests {
    ...
    #[test]
    fn skill_level_accepts_plus_and_multiply_strings() {
        assert_eq!(parse_skill_level(&serde_json::json!(40)), 40);
        assert_eq!(parse_skill_level(&serde_json::json!("40+30")), 70);
        assert_eq!(parse_skill_level(&serde_json::json!("2*22")), 44);
        assert_eq!(parse_skill_level(&serde_json::json!("bad")), 0);
    }

    #[test]
    fn canonical_skill_key_prefixes_skl() {
        assert_eq!(canonical_skill_key("sklfire"), "sklfire");
        assert_eq!(canonical_skill_key("Fire"), "sklfire");
        assert_eq!(canonical_skill_key(" fire "), "sklfire");
    }
}
