...
impl CaseMode {
    ...
    fn build_input(self, players: &[String]) -> String {
        match self {
            ...
            Self::FreeForAll(_) => players.join("\n"),
        }
    }
}
...
fn load_cases(config: &Config) -> Result<Vec<GeneratedCase>, String> {
...
fn load_cases_dir(cases_dir: &Path) -> Result<Vec<GeneratedCase>, String> {
    if !cases_dir.is_dir() {
...
fn generate_cases_for_mode(names: &[String], mode: CaseMode, case_offset: usize, max_cases: usize) -> Vec<GeneratedCase> {
    ...
    for step in candidate_steps(names.len()) {
        for offset_idx in 0..names.len() {
            ...
            let id = format!("{}-{:016x}", mode.label(), input_hash);
            cases.push(GeneratedCase {
                id,
                mode: mode.label(),
                players,
...
fn run_case(case: &GeneratedCase, config: &Config) -> Result<CaseResult, String> {
