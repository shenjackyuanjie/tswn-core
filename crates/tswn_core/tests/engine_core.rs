...
mod bed2 {
    ...
    fn opening_round_summons_and_body_is_not_targetable() {
        let mut runner = Runner::new_from_namerena_raw(
        ...
        assert_eq!(
            first_updates
                .updates
                .iter()
                .filter(|update| !matches!(update.update_type, UpdateType::NextLine))
                .count(),
            2,
        ...
        let targets = tick::select_targets(enemy_id, &runner.world, &runner.storage);
        ...
        let damage = runner.storage.just_get_player_mut(bed2_id).unwrap().attacked(
            ...
            false,
            enemy_id,
            tswn_core::player::noop_on_damage,
            &mut randomer,
            &mut updates,
            &runner.storage,
        );
    ...
    fn opening_round_splits_multiple_bed2_summons_into_two_lines_each() {
        let mut runner =
            Runner::new_from_namerena_raw("alpha@red+bed2[3000]\nomega@red+bed2[3000]\n\nbeta@blue".to_string()).unwrap();
        ...
        assert!(
            ...
            "each bed2 opening row should contain one message"
        );
    ...
    fn resummons_after_own_summon_dies() {
        let mut runner = Runner::new_from_namerena_raw("alpha extra@red+bed2[3000]\n\nbeta@blue".to_string()).unwrap();
        let bed2_id = runner
            .world
            .all_plrs()
            .into_iter()
    ...
    fn body_damage_is_silent_until_blood_sacrifice_reports_hp() {
        let mut runner = Runner::new_from_namerena_raw("alpha@red+bed2[3000]\n\nbeta@blue".to_string()).unwrap();
        let bed2_id = runner
            .world
            .all_plrs()
            .into_iter()
