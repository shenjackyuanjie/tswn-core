...
- 新增服务端直播/回放高层接口：
  - `Runner.winner_team_index()` / `Runner.winner_team_indices()`：直接返回胜利输入队伍索引。
  - `Runner.snapshot_players()`：返回包含队伍归属、显示顺序、基础信息、属性和运行状态的标准玩家快照。
  - `RunUpdate.to_dict(rendered=True)`：返回结构化事件 DTO，并可同时给出渲染后的消息文本。
