...
impl BenchSummary {
    pub(crate) fn win_rate_percent(&self) -> f64 {
        self.wins as f64 * 100.0 / self.total.max(1) as f64
    }
}
...
fn run_bench_score_inner(
    ...
    let (wins, total, timing) = if workers <= 1 || n < BENCH_PARALLEL_THRESHOLD {
        ...
        for _ in 0..workers {
            ...
            handles.push(std::thread::spawn(move || run_bench_score_worker(&target_group, &modifier, &next, n)));
        }
        let mut wins = 0usize;
...
fn run_bench_score_range(
    ...
    for i in start..end {
        ...
        let mut runner = match Runner::new_from_groups_with_seed_and_eval_rq(
            &groups,
            &seed,
        ...
        let target_team: Vec<usize> = runner
            .input_groups
            .first()
