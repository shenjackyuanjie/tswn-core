...
- 补充 `pair` 的带权靶子、多人输入和两种分隔符的配置说明与示例。
...
- `cqd/cqp` 靶子预设新增可选配置 `factor_enabled`，省略时默认为 `false`；启用后从 TOML 靶子文件读取每组 `factor` 和 `players`。
- 新增内嵌的 50 组带权二人靶子 `targets/newTarget2.toml`，默认配置中可通过“新版二人组”选择。

### 调整

- 带权 `cqd/cqp` 的平均胜率改为按有效靶子的权重计算：`sum(胜率 * factor) / sum(factor)`。
- 带权模式下，双方玩家完全相同时直接按 `50%` 参与加权；仅部分玩家相同时正常计算，不再因重名跳过。未启用带权模式时保持原逻辑。

### 验证

- `cargo check -p tswn_openbox`
- `cargo test -p tswn_openbox`
- 完整带权资源解析、同阵容 `50%`、部分重名正常计算的专项测试。
