...
//! 该行为用于复刻原版 DS4 工具“先合并新增输入再进入去重/排序”的处理链。
...
pub fn merge_input_files(input_dir: &Path, output_file: &Path) -> Ds4Result<usize> {
    ...
    for path in entries {
        io::copy(&mut fs::File::open(&path)?, out)?;
        merged_files += 1;
    }
...
mod tests {
    ...
    fn merge_files_in_lexicographic_order() {
        let root = temp_dir("tswn-ds4-merge");
