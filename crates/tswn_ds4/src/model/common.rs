//! 单字评分文件处理与潜力分估算。
//!
//! 这里承接各具体模型入口传入的 `ScoreMode` 与系数数组，负责读取名字、
//! 计算基础分和可提升到的潜力分，再按阈值写出筛选结果。
//! 多线程版本保持输入顺序稳定，避免并行评分改变输出文件语义。
...
pub fn score_file(
    ...
    let input: Box<dyn BufRead> = if input_file.exists() {
...
pub fn score_file_with_threads(
    ...
    let pool = ThreadPoolBuilder::new()
        .num_threads(threads)
        .build()
        .map_err(|err| Ds4Error::parse(format!("failed to build score thread pool: {err}")))?;
    let mut writer = AtomicFileWriter::new(output_file)?;
    let mut written = 0;
    let input: Box<dyn BufRead> = if input_file.exists() {
        Box::new(BufReader::new(fs::File::open(input_file)?))
    ...
    for raw_line in input.lines() {
        ...
        if names.len() == 1024 {
            written += write_scored_chunk(&pool, &names, writer.writer(), mode, score_sieve, potential_sieve, model, ctx)?;
            names.clear();
        }
    }
    if !names.is_empty() {
...
fn write_scored_chunk(
    ...
    let rows = pool.install(|| {
        names
            .par_iter()
            .map(|name| evaluate_name(name, mode, model, ctx).ok())
            .collect::<Vec<_>>()
    });
    let mut written = 0;
    for (name, scored) in names.iter().zip(rows) {
        if let Some(scored) = scored
            && (scored.score >= score_sieve as f64 || scored.potential >= potential_sieve as f64)
        {
            write!(output, "{:.0} {:.0} {name}\r\n", scored.score, scored.potential)?;
            written += 1;
        }
    }
    Ok(written)
...
pub fn evaluate_name(full_name: &str, mode: ScoreMode, model: &[f64; MODEL_SIZE], ctx: &ScoreContext) -> Ds4Result<SingleScore> {
