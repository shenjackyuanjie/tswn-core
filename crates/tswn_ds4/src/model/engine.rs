...
impl ScoreMode {
    ...
    pub fn from_i32(value: i32) -> Ds4Result<Self> {
        match value {
            1 => Ok(Self::Bc),
            2 => Ok(Self::Fz),
            3 => Ok(Self::Wc),
            4 => Ok(Self::Fs),
            5 => Ok(Self::Pj),
            _ => Err(Ds4Error::parse(format!("invalid score mode: {value}"))),
        }
    }
}

pub type PositionTables = ([usize; 46], [[Option<usize>; 46]; 46]);

#[allow(clippy::needless_range_loop)]
pub fn init_pos_tables() -> PositionTables {
...
impl NameFeature {
    pub fn from_full_name(full: &str) -> Ds4Result<Self> {
        let bytes = full.as_bytes();
        if bytes.is_empty() {
            return Err(Ds4Error::parse("invalid full name: empty"));
        }
        ...
        if l > r {
            return Err(Ds4Error::parse(format!("invalid full name: {full}")));
        }
        ...
        let at_pos = at_pos.ok_or_else(|| Ds4Error::parse(format!("name lacks @team: {full}")))?;
        ...
        let shadow_props = [
            median(shadow.name_base[10], shadow.name_base[11], shadow.name_base[12]) as f64,
            median(shadow.name_base[13], shadow.name_base[14], shadow.name_base[15]) as f64,
            median(shadow.name_base[16], shadow.name_base[17], shadow.name_base[18]) as f64,
            median(shadow.name_base[19], shadow.name_base[20], shadow.name_base[21]) as f64,
            median(shadow.name_base[22], shadow.name_base[23], shadow.name_base[24]) as f64,
            median(shadow.name_base[25], shadow.name_base[26], shadow.name_base[27]) as f64,
            median(shadow.name_base[28], shadow.name_base[29], shadow.name_base[30]) as f64,
            {
                let mut hp = shadow.name_base[..10].to_vec();
                hp.sort_unstable();
                (154 + hp[3] as i32 + hp[4] as i32 + hp[5] as i32 + hp[6] as i32) as f64
            },
        ];
        // SP1 与 SP2 的 shadowi 权重不同，两个值分别保留，避免改变五类基础评分。
        let shadowi_sp1 = shadow_props[0] * 2.8
            + shadow_props[1] * 0.6
            + shadow_props[2] * 2.5
            + shadow_props[3] * 1.2
            + shadow_props[4]
