...
pub fn parse_args<I>(args: I) -> Ds4Result<Cli>
where
    I: IntoIterator<Item = String>,
{
    ...
    match cmd.as_str() {
        ...
        _ => Err(Ds4Error::cli(format!("unknown command: {cmd}"))),
    }
}

fn parse_run(args: Vec<String>) -> Ds4Result<Cli> {
    ...
    while idx < args.len() {
        match args[idx].as_str() {
            "--root" => {
                let value = args.get(idx + 1).ok_or_else(|| Ds4Error::cli("missing value after --root"))?;
                root = PathBuf::from(value);
                idx += 2;
            }
            "--config" => {
                let value = args.get(idx + 1).ok_or_else(|| Ds4Error::cli("missing value after --config"))?;
                config_path = Some(PathBuf::from(value));
                idx += 2;
            }
            unknown => return Err(Ds4Error::cli(format!("unknown run arg: {unknown}"))),
        }
    }
...
fn parse_show_config(args: Vec<String>) -> Ds4Result<Cli> {
    let mut root = std::env::current_dir()?;
    let mut config_path = None;
    ...
    while idx < args.len() {
        match args[idx].as_str() {
            ...
            unknown => return Err(Ds4Error::cli(format!("unknown show-config arg: {unknown}"))),
        }
    }
...
fn parse_merge(args: Vec<String>) -> Ds4Result<Cli> {
    if args.len() != 2 {
        return Err(Ds4Error::cli("merge usage: tswn_ds4 merge <input_dir> <output_file>"));
    }
    Ok(Cli {
...
fn parse_dedup(args: Vec<String>) -> Ds4Result<Cli> {
    if args.len() != 3 {
        return Err(Ds4Error::cli("dedup usage: tswn_ds4 dedup <new_file> <old_file> <output_file>"));
    }
    Ok(Cli {
...
fn parse_sort(args: Vec<String>) -> Ds4Result<Cli> {
    if args.len() != 5 {
        return Err(Ds4Error::cli(
            "sort usage: tswn_ds4 sort <output_score_0_or_1> <input_file> <output_file> <score_number> <sort_key_1_based>",
        ));
    }
    let output_score = match args[0].as_str() {
        "0" => false,
        "1" => true,
        other => return Err(Ds4Error::cli(format!("output_score must be 0 or 1, got {other}"))),
    };
    let score_number = args[3]
        .parse::<usize>()
        .map_err(|_| Ds4Error::cli(format!("invalid score_number: {}", args[3])))?;
    let sort_key_one_based = args[4]
        .parse::<usize>()
        .map_err(|_| Ds4Error::cli(format!("invalid sort_key: {}", args[4])))?;
    if score_number == 0 {
        return Err(Ds4Error::cli("score_number must be >= 1"));
    }
    if sort_key_one_based == 0 || sort_key_one_based > score_number {
        return Err(Ds4Error::cli(format!(
...
fn parse_score(args: Vec<String>, mode: SingleMode) -> Ds4Result<Cli> {
    if args.len() != 5 {
        return Err(Ds4Error::cli(
            "score usage: tswn_ds4 score-<bc|fz|wc|fs|pj> <threads> <input_file> <output_file> <score_sieve> <potential_sieve>",
        ));
    }
    let threads = args[0]
        .parse::<usize>()
        .map_err(|_| Ds4Error::cli(format!("invalid threads: {}", args[0])))?;
    let score_sieve = args[3]
        .parse::<i32>()
        .map_err(|_| Ds4Error::cli(format!("invalid score_sieve: {}", args[3])))?;
    let potential_sieve = args[4]
        .parse::<i32>()
        .map_err(|_| Ds4Error::cli(format!("invalid potential_sieve: {}", args[4])))?;
    Ok(Cli {
...
fn parse_pair(args: Vec<String>, mode: PairCliMode) -> Ds4Result<Cli> {
    if args.len() != 6 {
        return Err(Ds4Error::cli(
            "pair usage: tswn_ds4 pair-<fc|wc|rh> <type_0_or_1> <threads> <sieve> <left_file> <right_file> <output_file>",
        ));
    }
    let type_same_set = match args[0].as_str() {
        "0" => true,
        "1" => false,
        other => return Err(Ds4Error::cli(format!("invalid pair type: {other}, must be 0 or 1"))),
    };
    let threads = args[1]
        .parse::<usize>()
        .map_err(|_| Ds4Error::cli(format!("invalid threads: {}", args[1])))?;
    let sieve = args[2].parse::<i32>().map_err(|_| Ds4Error::cli(format!("invalid sieve: {}", args[2])))?;
    Ok(Cli {
...
NOTES:
  ...
  - Config lookup order in root: config.json -> config.toml -> config.txt
