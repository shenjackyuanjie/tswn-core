...
fn bin_path() -> PathBuf {
    let exe = if cfg!(windows) { "tswn_ds4.exe" } else { "tswn_ds4" };
    PathBuf::from(env!("CARGO_BIN_EXE_tswn_ds4")).parent().expect("binary parent").join(exe)
}
...
fn prepare_fixture_work(fixture_name: &str) -> (PathBuf, PathBuf) {
    ...
    let work = temp_dir(&format!("tswn-ds4-pipeline-{fixture_name}"));
...
fn golden_pipeline_basic_matches_expected() {
    ...
    run_pipeline(&work);
    assert_expected_tree(&work, &expected, &["tmp", "file", "new"]);
    let fc = fs::read_to_string(work.join("out/FC.txt")).expect("read FC output");
    assert!(
        fc.lines().count()
            > fs::read_to_string(expected.join("out/FC.txt"))
                .expect("read old FC baseline")
                .lines()
                .count()
    );
...
fn pipeline_outputs_stable_between_threads_1_and_4() {
    ...
    assert_expected_tree(&work1, &expected, &["tmp", "file", "new"]);
    assert_expected_tree(&work4, &expected, &["tmp", "file", "new"]);
    for dir in ["tmp", "file", "new"] {
    ...
    for name in ["FC.txt", "WC.txt", "RH.txt"] {
        assert_same_file(&work1.join("out").join(name), &work4.join("out").join(name));
    }
