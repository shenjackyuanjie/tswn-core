...
fn run_and_expect_success(args: &[&str]) {
    let output = std::process::Command::new(bin_path()).args(args).output().expect("run tswn_ds4 command");
    assert!(
        output.status.success(),
        "command failed: tswn_ds4 {}\nstdout:\n{}\nstderr:\n{}",
        args.join(" "),
...
fn golden_single_outputs_match_fixture() {
    ...
    let work = temp_dir("tswn-ds4-golden-single");
...
fn single_threads_1_and_4_are_identical() {
    let fixture = fixture_dir("single");
    let work = temp_dir("tswn-ds4-single-threads");
    let input = fixture.join("input").join("names.txt");
